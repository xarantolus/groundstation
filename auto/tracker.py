...
def fetch_tle(norad: str, n2yo_api_key: str) -> Tuple[str, str]:
    try:
        ...
        if n2yo_api_key:
            r = requests.get(
                f"https://api.n2yo.com/rest/v1/satellite/tle/{norad}/?apiKey={n2yo_api_key}",
                timeout=10,
            )
            r.raise_for_status()
...
def get_next_passes(
    ...
    while current_date < end_date:
        try:
            ...
            current_date = (
                set_time + ephem.minute
            )  # Add a minute to avoid finding same pass
...
def stage_directory(
    ...
    for root, _, files in os.walk(pass_dir):
        for file in files:
            ...
            if sat.get("skip_iq_upload", False) and file.endswith(
            ...
            logging.info(
                f"Queued file for transfer: {os.path.basename(file)} | "
    ...
    while True:
        try:
            ...
            while (
                NEXT_OVERPASS
                and (
                    NEXT_OVERPASS["start_time"] - datetime.datetime.now()
                ).total_seconds()
                < 300
            ):
                time_until_end = (
                    NEXT_OVERPASS["end_time"] - datetime.datetime.now()
                ).total_seconds()

                logging.info(
                    f"Decoder: Next overpass is within 5 minutes, waiting at least {time_until_end:.1f} seconds until it is over before decoder start"
                )
            ...
            decoders: List[Decoder] = (
                satellite["decoder"]
                if isinstance(satellite["decoder"], list)
                else [satellite["decoder"]]
            )
            for decoder in decoders:
                try:
                    decoder_name = decoder.get("name")
                    logging.info(
                        f"Running decoder {(decoder_name + ' ') if decoder_name else ''}for {satellite['name']} in {pass_dir}"
                    )
                    run_decoder(satellite, decoder, pass_dir)
                    logging.info(
                        f"Decoder {(decoder_name + ' ') if decoder_name else ''}for {satellite['name']} completed successfully"
                    )
                except Exception as e:
                    logging.error(
                        f"Error running decoder {(decoder_name + ' ') if decoder_name else ''} for {satellite['name']}: {e}"
                    )
                    continue

            files = stage_directory(queue, nas_dir, pass_dir, pass_info, satellite)
            logging.info(
                f"Decoding complete for {satellite['name']} in {pass_dir}, queued {len(files)} files for transfer to NAS"
            )
...
def pass_to_string(sat: Satellite, pass_info: PassInfo) -> str:
    ...
    return (
        f"{sat['name'].ljust(20)} {pass_info['start_time'].date()} {start_time_str} - {end_time_str} | "
        + f"{duration_str} min | Max Elevation: {pass_info['max_elevation']:.1f}° | "
        + f"{start_az} -> {max_az} -> {end_az}"
    )


def prioritize_overlapping_passes(
    ...
    while current_index < len(next_passes):
        ...
        if overlapping_passes:
            ...
            priority_candidates = [
                (i, sat, pass_info)
                for i, sat, pass_info in overlapping_passes
            ...
            if len(overlapping_passes) > 1:
                logging.info(
                    f"Found {len(overlapping_passes)} overlapping passes (selected by {selection_criterion}):"
                )
                for idx, sat, pass_info in overlapping_passes:
                    priority_str = (
                        f" | Priority: {sat.get('priority', 'None')}"
                        if "priority" in sat
                        else ""
                    )
                    status = "SELECTED" if idx == best_idx else "SKIPPED "
                    logging.info(
                        f"  {status}: {pass_to_string(sat, pass_info)}{priority_str}"
                    )
...
def main() -> None:
    ...
    transfer_thread = threading.Thread(
        target=file_transfer_worker, daemon=True, args=(transfer_queue,)
    )
    transfer_thread.start()
    decode_thread = threading.Thread(
        target=decode_worker, daemon=True, args=(transfer_queue, nas_dir)
    )
    decode_thread.start()

    logging.info(
        f"Loaded config, tracking {len(sats)} satellites, logging to {nas_dir}"
    )

    while True:
        next_passes: List[Tuple[Satellite, PassInfo]] = []
        logging.info("Checking for next passes...")
        ...
        for sat in sats:
            try:
                ...
                logging.info(
                    f"Fetched TLE for {sat['name']}: {tle1.strip()} | {tle2.strip()}"
                )
                passes = get_next_passes(
                    ...
                    geo,
                    threshold,
                    pass_start_threshold,
                    hours=int(1.25 * update_interval / 3600),
                )

                logging.info(
                    f"Found {len(passes)} passes for {sat['name']} in the next {update_interval / 3600:.1f} hours"
                )
                ...
                if not passes:
                    logging.warning(
                        f"No viable passes for {sat['name']} in next 24 hours"
                    )
            except Exception as e:
        ...
        if not next_passes:
            logging.info(
                f"No passes found in the next {update_interval / 3600:.1f} hours, sleeping for 15 minutes..."
            )
            time.sleep(15 * 60)
            continue
        ...
        for i, (sat, pass_info) in enumerate(next_passes):
            ...
            if pass_info["end_time"] < datetime.datetime.now():
                logging.info(
                    f"Pass for {sat['name']} is already in the past ({pass_info['start_time']}), skipping..."
                )
                continue

            # If the pass is too far in the future, skip it
            if pass_info["start_time"] > datetime.datetime.now() + datetime.timedelta(
                seconds=update_interval
            ):
                logging.info(
                    f"Pass for {sat['name']} is too far in the future, skipping..."
                )
                continue
            ...
            if wait_time > 0:
                logging.info(
                    f"Waiting {wait_time:.1f} seconds until pass of {sat['name']} starts..."
                )
                ...
                while time_waited < wait_time:
                    ...
                    if time_left > 30:
                        logging.info("Next passes:")
                        for next_sat, next_pass_info in next_passes[i : i + 5]:
                            logging.info(
                                f"  {pass_to_string(next_sat, next_pass_info)}"
                            )
                        logging.info(
                            f"Waiting {time_left:.1f} seconds ({time_left / 60:.1f} minutes) until pass of {sat['name']}..."
                        )
            ...
            logging.info(
                f"Starting pass for {sat['name']} with max elevation {pass_info['max_elevation']:.1f}°..."
            )
            ...
            try:
                ...
                with open(pass_info_file, "w") as f:
                    ...
                    for key in ["start_time", "max_time", "end_time"]:
                        pass_info_serializable[key] = pass_info_serializable[
                            key
                        ].isoformat()
                ...
                if "decoder" in sat and sat["decoder"] is not None:
                    ...
                    files = stage_directory(
                        transfer_queue, nas_dir, tmp_dir, pass_info, sat
                    )
                    logging.info(
                        f"Recording complete, queued {len(files)} files for transfer to NAS"
                    )
            except Exception as e:
